...
当前 API 服务无任何鉴权，局域网或公网部署时存在安全风险。

- [ ] `PageSettings` 新增 API Key 配置项，保存到 `data/settings.json`
- [ ] `server.ts` 添加中间件，校验 `Authorization: Bearer <key>`
- [ ] 未设置 Key 时跳过校验，保持向后兼容
- [ ] 管理面板 API（admin 端口）独立鉴权或共用

预计工作量：0.5 天

### Docker 部署

Web 模式天然适合容器化，降低服务器部署门槛。

- [ ] 编写 `Dockerfile`（基于 `node:20-slim`，多阶段构建）
- [ ] 编写 `docker-compose.yml`（挂载 `data/` 目录持久化凭证）
- [ ] 支持环境变量配置端口（`PORT`、`ADMIN_PORT`）
- [ ] README 补充 Docker 部署说明

预计工作量：0.5 天

### 监听地址可配置

API 服务当前硬编码绑定 `127.0.0.1`，无法局域网访问。

- [ ] `PageSettings` 新增监听地址选项（`127.0.0.1` / `0.0.0.0`）
- [ ] 配合 API Key 鉴权使用，`0.0.0.0` 模式下强制要求设置 Key
- [ ] `server-standalone.ts` 支持 `--host` 参数
...
- [ ] Provider 内部维护账号池，支持 Round-Robin / 随机策略
- [ ] 单账号请求失败时自动切换下一个
- [ ] 管理面板支持添加/删除/排序多组凭证
...
- [ ] 后台定时检测 Token 过期状态（复用现有 `checkExpiry()`）
- [ ] 快过期时（剩余 < 10 分钟）自动触发 `captureCredentials()`
- [ ] 刷新失败时通知用户（系统托盘气泡 / 管理面板告警）
- [ ] 可配置自动刷新开关和提前量
...
- [ ] 每个 Provider 维护独立请求队列
- [ ] 可配置每分钟最大请求数（按 Provider 独立设置）
- [ ] 超限时排队等待，返回 `429` 或等待后重试
- [ ] 管理面板展示队列状态（排队数 / 处理中）
